...
class DBTMime {
	...
	protected static $ItemsLen = 3;
	protected static $Table = array( array(), array(), array() );
	// Returns array(name, mimetype, extension)
	public static function From(int $aFrom, string $aStr) {
		// $aFrom [0=name, 1=mimetype, 2=extension]
		$aStr = strtolower($aStr);
		$aFrom = abs($aFrom % 3);
		$fi = -1;
		for ($i = 0, $L = count(self::$Table[$aFrom]); $i < $L; $i++) {
			$item = self::$Table[$aFrom][$i];
			if ($aStr === $item) {
				$fi = $i;
				break;
			}
		...
		if (-1 !== $fi)
			return array_slice(self::$Mimes, $fi*self::$ItemsLen, self::$ItemsLen);
		return null;
	}
	// Used for initialization only
	public static function SetMimes($aArray) {
		if (!count(self::$Mimes)) {
			self::$Mimes = $aArray;
			for ($i = 0, $L = floor(count($aArray)/self::$ItemsLen); $i < $L; $i++) {
				$ri = $i * self::$ItemsLen;
				$name = $aArray[0 + $ri];
				$mime = $aArray[1 + $ri];
				$ext = $aArray[2 + $ri];
				self::$Table[0][] = strtolower($name);
				self::$Table[1][] = strtolower($mime);
				self::$Table[2][] = strtolower($ext);
			}
		}
	}
}
DBTMime::SetMimes([
	...
	"les",
	"Archive document - Multiple Fils Embedded",
